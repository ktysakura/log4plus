...
log4cplus::tstring
getPropertiesFileArgument01 (int argc, char * argv[])
{
    if (argc >= 2)
    {
    ...
    return LOG4CPLUS_TEXT ("16_asyncappender.properties");
}

// g++ 
int log4cpp_async(int argc, char * argv[])
{
    tcout << LOG4CPLUS_TEXT("Entering main()...") << endl;
    log4cplus::Initializer initializer;

    PropertyConfigurator::doConfigure(getPropertiesFileArgument01(argc, argv));
    ...
    try {
        ...
        tstring msg(LOG4CPLUS_TEXT("This is a WARNING... i:"));
        int i = 0;
        for(i=0; i<LOOP_COUNT; ++i) {
            LOG4CPLUS_WARN(logger, msg << i);
        }
        hr_clock::time_point end = hr_clock::now ();
        hr_clock::duration diff = end - start;
        double diff_seconds = sec_dur_type (diff).count ();
        LOG4CPLUS_WARN(LOG4CPLUS_TEXT("root"), "Logging " << LOOP_COUNT << " took: " << diff_seconds << "(s)" << endl);
        LOG4CPLUS_WARN(root, "Logging average: " << (diff_seconds/LOOP_COUNT) << endl);
        ...
        LOG4CPLUS_WARN(root, "tostringstream average: " << (diff_seconds/LOOP_COUNT) << "(s)" << endl);
