...
log4cplus::tstring
getPropertiesFileArgument02(int argc, const char * argv)
{
    if (argc >= 2)
    {
		const char * arg = argv;
    ...
    return LOG4CPLUS_TEXT ("15_watch.properties");
}

// 就是修改15_watch.properties的文件，log4cplus能够自动检测到
// 编译：g++ -o 15_watch 15_watch.cpp -llog4cplus -std=c++11
// 运行：./15_watch
int log4cpp_watch()
{
	int argc = 2;
	const char *argv = "conf/15_watch.properties";
    ...
    try
    {
        ConfigureAndWatchThread configureThread(
			getPropertiesFileArgument02(argc, argv), 5 * 1000);
